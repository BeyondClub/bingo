...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const campaign = await db.campaigns.findFirst({
        where: {
            contract_address: "0x00583DcbF4298b082ac796Ec7833F4ea83A5388C"
        }
    });
    const key = {
        tokenId: 1,
        owner: '0x00583DcbF4298b082ac796Ec7833F4ea83A5388C'
    }
    ...
    if (campaign) {
        ...
        if (bingoExist === 0) {
            ...
            const campaign_tasks: campaigns_tasks[] = await db.$queryRawUnsafe(
                `SELECT * FROM "campaigns_tasks" WHERE campaign_id = '${campaign?.campaign_id}' ORDER BY ${campaign.random_grid ? 'RANDOM()' : '"order" ASC'} LIMIT 24`
            );
